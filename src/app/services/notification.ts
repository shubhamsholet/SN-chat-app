...
export class NotificationService {
  ...
  private async processUserUpdates(userData: any, currentUserUID: string): Promise<void> {
    ...
    await this.processMessageNotifications(userData, currentUserUID);
    
    // Process system notifications
    await this.checkForSystemNotifications(userData);
  }

  private async processMessageNotifications(userData: any, currentUserUID: string): Promise<void> {
    if (!userData.allChatList || !Array.isArray(userData.allChatList)) return;

    const previousChatList = this.localStorage.getObject<any[]>('previous-chat-list') || [];

    for (const chat of userData.allChatList) {
      const previousChat = previousChatList.find((prev: any) => prev && prev.partnerUID === chat.partnerUID);
      
      if (this.shouldNotifyNewMessage(chat, previousChat, currentUserUID)) {
        await this.createMessageNotification(chat);
      }
    }

    // Update stored chat list
    this.localStorage.setItem('previous-chat-list', userData.allChatList);
  }

  private shouldNotifyNewMessage(currentChat: any, previousChat: any, currentUserUID: string): boolean {
    if (!currentChat.lastMeassage) return false;

    // Check if message is from partner
    const isMessageFromPartner = this.isMessageFromPartner(currentChat.lastMeassage, currentUserUID);
    if (!isMessageFromPartner) return false;
    ...
    // Existing chat with new message
    if (previousChat && previousChat.lastMeassage) {
      const previousTime = new Date(previousChat.lastMeassage.time).getTime();
      const currentTime = new Date(currentChat.lastMeassage.time).getTime();
      return currentTime > previousTime;
  ...
  private isMessageFromPartner(lastMessage: any, currentUserUID: string): boolean {
    ...
    if (lastMessage.type === 'other' || lastMessage.type === 'received') return true;
    if (lastMessage.type === 'sent' || lastMessage.type === 'me') return false;
    
    return true; // Default to partner if uncertain
  }
  ...
  private async createMessageNotification(chat: any): Promise<void> {
    ...
    // to store in local storage and notification shell
    // await this.storeNotification(notificationData);
  }
  ...
  private async checkForSystemNotifications(userData: any): Promise<void> {
    ...
    // Check for any other notification fields in user data
    if (userData.notifications && Array.isArray(userData.notifications)) {
      await this.processFirestoreNotifications(userData.notifications);
    }
  }
    ...
    for (const request of friendRequests) {
      ...
      if (isNewRequest) {
        await this.addNotification({
          type: 'friend_request',
          title: 'Friend Request',
          body: `${request.fromUserName || 'Someone'} sent you a friend request`,
          metadata: {
            requestId: request.requestId,
            fromUserId: request.fromUserId,
            fromUserName: request.fromUserName
  ...
  private async processFirestoreNotifications(firestoreNotifications: any[]): Promise<void> {
    const previousNotifications = this.localStorage.getObject<any[]>('previous-firestore-notifications') || [];
    
    for (const notif of firestoreNotifications) {
      const isNewNotification = !previousNotifications.some(prev => 
        prev.id === notif.id
      );

      if (isNewNotification && !notif.read) {
        await this.addNotification({
    ...
    // Update stored Firestore notifications
    this.localStorage.setItem('previous-firestore-notifications', firestoreNotifications);
  }

  // Public method to manually add notifications
  async addNotification(notificationConfig: {
    type: 'system' | 'friend_request' | 'other' | 'message';
    title: string;
    body: string;
    metadata?: { [key: string]: any };
  }): Promise<void> {
